...
# ╔═╡ 1fa87a7e-c06e-11f0-11bf-ad9021f447c0
begin
    using WGLMakie
    using Bonito
...
# ╔═╡ cce5ceee-988a-4d89-b77d-2d0c435296fd
using PlutoUI

# ╔═╡ 2091e389-15b9-4c0e-8eeb-450ed8efea38
using PlutoTeachingTools

# ╔═╡ 6c003160-b9f3-4f55-831c-7b61db003ffb
html"<button onclick='present()'>Slide mode</button>"

# ╔═╡ 56e9734e-5262-45cb-a340-0a47fc32c08c
md"""
# Pluto test
**Author**
...
# ╔═╡ 3e064916-6109-47e1-96e3-e5b03471c131
md"## Slide 1"

# ╔═╡ 08a45753-58a8-464d-8de5-945007baca22
md"## Slide 2"

# ╔═╡ 2bf8868f-9d06-41b9-9838-49cf97c8d572
md"# Pluto Appendix"

# ╔═╡ dcc6a8b8-f523-471a-8cf1-011a325ec5fa
import MarkdownLiteral: @markdown

# ╔═╡ 8a51f8fc-9ad9-439c-90a5-3ca00896536f
begin
	FIGURE_RESOLUTION = (1400, 850)
end;

# ╔═╡ ad188d58-b586-4cb8-8e8d-d0547df82d44
App() do
    ...
    begin # Animation setup
        # Figure setup
        fig = Figure(size=FIGURE_RESOLUTION, backgroundcolor=:black)
...
# ╔═╡ 6468e3fe-fe10-4b92-8fe6-cafd97a6a685
App() do

    # Constants for layout and geometry
    TRIANGLE_RADIUS = 2.0      # Distance from triangle the origin to the three planets
    CIRCLE_RADIUS = 0.5        # Radius of each circle
    ZOOM_PADDING = 2.0         # Padding factor around zoomed circle
    ANIMATION_DURATION = 1.0   # Animation duration in seconds
    ...
    begin # Initialize figure
        ...
        ax = Axis(fig[1, 2], aspect=AxisAspect(1))
        hidedecorations!(ax)
        hidespines!(ax)
        ...
        centers = [
            ...
            Point2f(-TRIANGLE_RADIUS * sqrt(3) / 2, -TRIANGLE_RADIUS / 2),
            Point2f(TRIANGLE_RADIUS * sqrt(3) / 2, -TRIANGLE_RADIUS / 2)
        ]

        # Strokecolors and labels for the circles
        colors = [:blue, :green, :red, :purple]
        labels = Observable(collect(INIT_LABELS))

        # Draw circles with centered text labels
        for i in 1:4
            poly!(ax, Circle(centers[i], CIRCLE_RADIUS),
                color=:white,
                strokecolor=WGLMakie.Colors.JULIA_LOGO_COLORS[colors[i]],
                strokewidth=2)

            text!(ax, centers[i],
                text=lift(x -> x[i], labels),
                align=(:center, :center),
                fontsize=12,
                color=:black,
                font=:bold)
        ...
        # Set initial limits to show all circles
        max_extent = TRIANGLE_RADIUS + CIRCLE_RADIUS
        initial_xlims = (-max_extent * 1.1, max_extent * 1.1)
        initial_ylims = (-max_extent * 1.1, max_extent * 1.1)
        xlims!(ax, initial_xlims)
        ylims!(ax, initial_ylims)

        # Create selection menu
        menu = Menu(fig[1, 1],
            options=["Show All", "PP/Neuro", "Probability", "PDE", "Statistics"],
            default="Show All", tellheight=false)

        # Animation state
        is_animating = false
        current_xlims = initial_xlims
        current_ylims = initial_ylims
    end

    # Animation function with lock to prevent concurrent animations
    function animate_limits!(target_x, target_y)
    ...
    function animate_limits_2steps!(target_x, target_y)
        ...
        @async begin
            # Reset view
            animate_limits!(initial_xlims, initial_ylims)
            # Move view
            animate_limits!(target_x, target_y)
            notify(labels)
        end
    end

    # Connect menu selection to zoom action
    on(menu.i_selected) do i_selected
        # Reset labels
        labels[] .= INIT_LABELS
        notify(labels)

        # Determine target limits
        if i_selected == 1 || isnothing(i_selected)
            target_x = initial_xlims
            target_y = initial_ylims
        else
            circle_num = i_selected - 1
            ...
            pad = CIRCLE_RADIUS * ZOOM_PADDING
            target_x = (center[1] - pad, center[1] + pad)
            target_y = (center[2] - pad, center[2] + pad)
        end
...
manifest_format = "2.0"
project_hash = "a957d10fd9e79e2b3ca394dc62ffd5a97cc408f3"
