...
begin # plot
    ...
    stairs!(axleft, [0.0; Ñ; tmax], [0:length(Ñ); length(Ñ)], alpha=0.7, color=:orange)

    N, Ñ = coupling(ipp, ipptilde, domain)
    stairs!(axright, [0.0; N; tmax], [0:length(N); length(N)], alpha=0.7, color=:darkblue)
    stairs!(axright, [0.0; Ñ; tmax], [0:length(Ñ); length(Ñ)], alpha=0.7, color=:orange)

    fig
end

# save figure
#save("plots/coupling-point-SDE.png", fig, px_per_unit=2)